...
func DeletebucketinventoryconfigurationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["inventory"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("inventory=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/%s#inventory&id%s", cfg.BaseURL, Bucket, queryString)
...
func CreateDeletebucketinventoryconfigurationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_Bucket#inventory&id",
		...
		mcp.WithBoolean("inventory", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletebucketinventoryconfigurationHandler(cfg),
	}
}
