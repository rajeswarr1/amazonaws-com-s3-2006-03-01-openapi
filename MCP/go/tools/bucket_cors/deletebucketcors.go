...
func DeletebucketcorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["cors"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("cors=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/%s#cors%s", cfg.BaseURL, Bucket, queryString)
...
func CreateDeletebucketcorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_Bucket#cors",
		...
		mcp.WithBoolean("cors", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletebucketcorsHandler(cfg),
	}
}
