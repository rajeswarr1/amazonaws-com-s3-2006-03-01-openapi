...
func DeletebucketencryptionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["encryption"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("encryption=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/%s#encryption%s", cfg.BaseURL, Bucket, queryString)
...
func CreateDeletebucketencryptionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_Bucket#encryption",
		...
		mcp.WithBoolean("encryption", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletebucketencryptionHandler(cfg),
	}
}
