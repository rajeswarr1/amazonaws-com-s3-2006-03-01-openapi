...
func DeletebucketanalyticsconfigurationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["analytics"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("analytics=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/%s#analytics&id%s", cfg.BaseURL, Bucket, queryString)
...
func CreateDeletebucketanalyticsconfigurationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_Bucket#analytics&id",
		...
		mcp.WithBoolean("analytics", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletebucketanalyticsconfigurationHandler(cfg),
	}
}
